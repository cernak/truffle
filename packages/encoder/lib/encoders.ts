...
export class ProjectEncoder {
  ...
  public async forAddress(
    ...
    const deployedBytecode = await new ProviderAdapter(this.provider).getCode(address, "latest");
...
export class ContractEncoder {
   ...
   * **This method is asynchronous.**
   *
   * This method works similarly to [[ProjectEncoder.encodeTxNoResolution]].
   * However, instead of a Method object, it simply takes the ABI entry for
   * the transaction being prepared.  Note that the ABI must be one associated
   * with this contract, or the function will not work.
   *
   ...
   * for this is planned for future versions.
   *
   * See [[ProjectEncoder.encodeTxNoResolution]] for further details.
   *
   * @param abi The ABI entry for the transaction being prepared.
   */
  public async encodeTxNoResolution(
   ...
   * However, instead of an array of Method objects, it simply takes either an
   * array of the ABI entries for the possible overloads, or a string giving
   * the name of the function to resolve.
   *
   * If passing in an array of ABI entries, function ABI entries may be passed
   * to this method, not constructor ABI entries.  Note that the ABIs must be
   * one associated with this contract, or the function will not work.
   *
   * Note that use of the encoder for transactions to be sent to libraries is
   * presently not supported and may have unreliable results.  Limited support
   * for this is planned for future versions.
